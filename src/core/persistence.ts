...
import { Constructor, deepCopy } from "./utilities";

type ModelClass = Constructor<Model> & { entity: string };
 ...
 */
async function loadInto(model: ModelClass, db: LocalForage) {
 ...
 */
function enablePersistence(model: ModelClass, db: LocalForage) {
	useRepo(model)
		.piniaStore()
 ...
 */
export async function bind(model: ModelClass) {
